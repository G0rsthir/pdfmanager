...
import { FileFolderSelect, StatusSelect } from "./file";
...
export function MoveFilesDialog(props: {
  ...
  const { form, mutation } = useFormMutation({
    ...
    onSuccess: onMoved,
    successMessage: `Moved ${fileCount(files.length)}`,
  });
  ...
  return (
    <FormModal
      ...
      title="Move files"
      confirmBtnText="Move"
      confirmBtnType={AccessScope.LIBRARY_WRITE}
        ...
        children={({ state: fieldState, handleChange, handleBlur }) => (
          ...
          >
            <FileFolderSelect
              ...
              allowedFolderIds={targetsQ.data?.map((folder) => folder.id)}
...
export function TagFilesDialog(props: {
  ...
  const { form, mutation } = useFormMutation({
    ...
    onSuccess: onTagged,
    successMessage: `Updated tags on ${fileCount(files.length)}`,
  });
...
export function DeleteFilesDialog(props: {
  ...
  const { mutate: deleteRequest } = useAPIMutation({
    ...deleteFilesBulkMutation(),
    onSuccess() {
      showSuccessNotification(`${fileCount(files.length)} deleted`);
      onSuccess?.();
    },
    onApiError(error) {
      showErrorNotification("File deletion failed", error.message);
    },
  });
...
export function SetStatusFilesDialog(props: {
  ...
  const { form, mutation } = useFormMutation({
    ...
    onSuccess: onSuccess,
    successMessage: `Updated status on ${fileCount(files.length)}`,
  });
